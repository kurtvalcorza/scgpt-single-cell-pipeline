"""Static release-asset validation for the scGPT cell-state classification DIMER pipeline.
...
REPO_NAME = "scgpt-single-cell-pipeline"
...
EXPECTED_OUTPUTS = (
    ...
    "outputs/scgpt_single_cell_embeddings.csv",
    "outputs/scgpt_single_cell_evaluation_report.json",
    "outputs/scgpt_single_cell_predictions.csv",
    "outputs/scgpt_single_cell_adapter",
    "outputs/scgpt_single_cell_result.json",
)
CODE_MARKERS = (
    # Stage 4: sample from the pinned vocabulary, validation with encodability, stratified split
    "USE_BYOD = False",
    ...
    # Stage 5: binning, scale invariance, ceilings, rejections, truncation
    ...
    # Stage 6: embeddings, reproducibility, gene-order invariance
    ...
    # Stage 7: the masked-expression objective and the lineage probe
    "masked = pipe.predict_masked(",
    "probe = pipe.predict_masked_genes(cells_of, markers)",
    # Stage 8: zero-training and trivial baselines fitted on train only
    ...
    # Stage 9: bounded fine-tuning
    "adapt_result = pipe.adapt(",
    "trainable_layers=TRAINABLE_LAYERS",
    "learning_rate=LEARNING_RATE",
    # Stage 10: held-out evaluation with deltas
    ...
    # Stage 11: inference, artifact export, reload parity
    ...
    # Stage 12: provenance, including the vocabulary's second source
...
MARKDOWN_MARKERS = (
    ...
    "Dataverse",
    "not a usable imputer",
    "split by that grouping",
)
...
EXTERNAL_WEIGHT_BYTES: dict[int, str] = {
    1_589: "upstream .gitattributes of tdc/scGPT at acf749f (not staged)",
    207_861_754: "upstream scgpt_gh_repo_original_model.bin of tdc/scGPT at acf749f (not staged)",
}
EXTERNAL_WEIGHT_DIGESTS: dict[str, str] = {
    "0175d29e942817fc71ecbc68dad5e3056fec772083388bcd66e3e6785408a30c": "scGPT fine-tune adapter.safetensors (6,317,552 bytes) from the build smoke run",
}
